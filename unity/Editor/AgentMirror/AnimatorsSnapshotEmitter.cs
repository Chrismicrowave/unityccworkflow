...
/// </summary>
[InitializeOnLoad]
public static class AnimatorsSnapshotEmitter
{
    ...
    [MenuItem("Tools/AgentMirror/UnitySnapshot/Animators Snapshot", false, 53)]
    public static void EmitNow()
    {
        try
        {
            ...
            AgentMirrorConfig.SafeWriteAllText(AgentMirrorConfig.AnimatorsSnapshotPath, SerializeDump(dump));
        }
        catch (Exception ex)
        {
            Debug.LogWarning($"[AgentMirror] AnimatorsSnapshotEmitter failed: {ex.Message}");
        }
    }
...
/// <summary>Triggers AnimatorsSnapshotEmitter when .controller files are imported.</summary>

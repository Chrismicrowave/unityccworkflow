...
public static class EditorSnapshotEmitter
{
    ...
    {
        // Auto-watch only if snapshot files already exist (generated by UCCPack or menu).
        // First-time gen is triggered externally — avoids overhead in projects
        // that don't use the snapshot system (e.g. artist copies).
        if (!System.IO.File.Exists(AgentMirrorConfig.EditorSnapshotPath)) return;

        EditorApplication.hierarchyChanged += MarkDirty;
        EditorApplication.playModeStateChanged += _ => MarkDirty();
        Undo.postprocessModifications += OnUndoRedo;
        EditorApplication.delayCall += () => { _dirty = true; };
    ...
    {
        _skipPrefabScan = false;
        EmitInternal();
    }

    /// <summary>Fast emit — open scenes only, no prefabs. Used by auto-watch.</summary>
    public static void EmitScenesOnly()
    {
        _skipPrefabScan = true;
        EmitInternal();
    }

    /// <summary>Full emit including all prefab assets.</summary>

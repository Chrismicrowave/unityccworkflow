...
/// <summary>
/// UnitySnapshot — EditorSnapshot.
///
/// Scans every GameObject in open scenes and prefab assets, then produces a
/// complete snapshot at Library/AgentMirror/EditorSnapshot.json with:
///   - Every GameObject's components and all serialized field values
///   - Nested struct fields captured via full property path
///   - Object references resolved to StableId
///   - Component count per GO
///   - Reference index (who references who)
///
/// The lightweight SceneMirror.json provides hierarchy-only queries (50KB).
/// This file (~2MB) is loaded on-demand when field values are needed.
///
/// Triggers: hierarchy change, component edit, undo/redo, prefab import.
/// </summary>
[InitializeOnLoad]
public static class EditorSnapshotEmitter
{
    // ── Data model ──────────────────────────────────────────────────────
    ...
    [MenuItem("Tools/AgentMirror/Generate Editor Snapshot")]
    public static void EmitNow()
    {
        try
        {
            ...
            AgentMirrorConfig.SafeWriteAllText(AgentMirrorConfig.EditorSnapshotMdPath, md);
        }
        catch (Exception ex)
        {
            Debug.LogWarning($"[AgentMirror] EditorSnapshotEmitter failed: {ex.Message}");
        }
    }

...
/// Library/AgentMirror/ProjectIndex.json.
///
/// This replaces most list_files/Glob/search_files MCP calls by giving the
/// AI a complete snapshot of the project's asset tree.
///
/// Triggers: asset import, delete, move, and periodically.
/// </summary>
[InitializeOnLoad]
public static class ProjectIndexEmitter
{
    [Serializable]
    ...
    [MenuItem("Tools/AgentMirror/Generate Project Index")]
    public static void EmitNow()
    {
        try
        {
            ...
            AgentMirrorConfig.SafeWriteAllText(
                AgentMirrorConfig.OutputDir + "/ProjectIndex.json", json);

            // Trim Markdown to a compact summary (full index would be too large for context)
            string summary = $"# Project Index\n" +
            ...
            AgentMirrorConfig.SafeWriteAllText(
                AgentMirrorConfig.OutputDir + "/ProjectIndex.md", summary);
        }
        catch (Exception ex)
        {
            Debug.LogWarning($"[AgentMirror] ProjectIndexEmitter failed: {ex.Message}");
        }
    }
...
public class ProjectIndexPostprocessor : AssetPostprocessor
{
        ...
        string[] movedFromAssetPaths)
    {
        if (importedAssets.Length > 0 || deletedAssets.Length > 0 || movedAssets.Length > 0)
            ProjectIndexEmitter.OnAssetChanged();
    }
}
